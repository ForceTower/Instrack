...
abstract class StoryWatchDao : BaseDao<StoryWatch>() {
    ...
    @Query("SELECT SW.userPk AS pk, SW.timestamp AS timestamp, P.username AS username, P.picture AS picture, P.name AS name FROM StoryWatch AS SW INNER JOIN Story AS S ON SW.storyPk = S.pk INNER JOIN Profile P on SW.userPk = P.pk LEFT JOIN ProfileBond PB ON P.pk = PB.userPk WHERE SW.storyPk = :pk AND (PB.followsMe = 0 OR PB.followsMe IS NULL) AND S.userPk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) AND (PB.referencePk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) OR PB.referencePk IS NULL) ORDER BY SW.timestamp DESC")
    ...
    @Query("SELECT PB.iFollow AS iFollow, PB.followsMe as followsMe, count(SW.userPk) AS insight, SW.timestamp AS timestamp, P.* FROM StoryWatch AS SW INNER JOIN Story AS S ON SW.storyPk = S.pk INNER JOIN Profile P on SW.userPk = P.pk LEFT JOIN ProfileBond PB ON P.pk = PB.userPk WHERE (PB.followsMe = 0 OR PB.followsMe IS NULL) AND S.userPk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) AND (PB.referencePk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) OR PB.referencePk IS NULL) GROUP BY SW.userPk ORDER BY COUNT(SW.userPk) ASC")
