...
abstract class StoryWatchDao : BaseDao<StoryWatch>() {
    ...
    @Query("SELECT SW.userPk AS pk, SW.timestamp AS timestamp, P.username AS username, P.picture AS picture, P.name AS name FROM StoryWatch AS SW INNER JOIN Story AS S ON SW.storyPk = S.pk INNER JOIN Profile P on SW.userPk = P.pk INNER JOIN ProfileBond PB ON P.pk = PB.userPk WHERE SW.storyPk = :pk AND PB.followsMe = 0 AND S.userPk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) AND PB.referencePk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) ORDER BY SW.timestamp DESC")
    abstract fun getNotFollowerWatcherOf(pk: Long): Flow<List<StoryWatchProfileSimple>>

    @Query("SELECT SW.userPk AS pk, SW.timestamp AS timestamp, P.username AS username, P.picture AS picture, P.name AS name FROM StoryWatch AS SW INNER JOIN Story AS S ON SW.storyPk = S.pk INNER JOIN Profile P on SW.userPk = P.pk INNER JOIN ProfileBond PB ON P.pk = PB.userPk WHERE SW.storyPk = :pk AND PB.iFollow <> 1 AND S.userPk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) AND PB.referencePk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1) ORDER BY SW.timestamp DESC")
