...
abstract class PostCommentDao : BaseDao<PostComment>() {
    ...
    @Query("SELECT PC.* FROM PostComment PC INNER JOIN Post P ON PC.postPk = P.pk WHERE PC.createdAt >= :min AND P.userPk = (SELECT LP.userPk FROM LinkedProfile AS LP WHERE LP.selected = 1 LIMIT 1)")
    abstract fun getAllComments(min: Long): Flow<List<PostComment>>
}
